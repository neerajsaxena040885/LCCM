% Array of draws has dimension MASTERDRAWS x NP x NALTMAX.
...
% NALTMAX]. Call this as the Master Draws matrix 
% -----------------------------------------------------------------

function dr=makedraws
 
global NALTMAX NP NTAKES DRAWTYPE MASTERDRAWS

% Define an empty array to store the draws
dr=zeros(MASTERDRAWS,NP,NALTMAX);
...
if DRAWTYPE == 2 || DRAWTYPE == 3   % Halton draws
   ...
   k=1;
   while size(h,2) < NALTMAX       %size(h,2) returns the number of prime numbers. Since we have to maintain separate prime for each random coeff, thus checking if the number of primes is sufficient, i.e. Number of primes > Number of parameters to be estimated
       h=primes(k.*100);
       k=k+1;
   end
   h=h(1,1:NALTMAX);
